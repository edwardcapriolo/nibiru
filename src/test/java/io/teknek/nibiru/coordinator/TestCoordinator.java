...
public class TestCoordinator {
    ...
    private void createMetaData(Server[] s) throws ClientException, InterruptedException {
        MetaDataClient c = getMetaDataClient(s);
        Map<String, Object> x = new HashMap<>();
        x.put(StoreMetaData.IMPLEMENTING_CLASS, DefaultColumnFamily.class.getName());
        c.createOrUpdateStore("abc", "def", x, true);
        Thread.sleep(10);
        for (Server server : s) {
            Assert.assertTrue(server.getKeyspaces().containsKey("abc"));
            Assert.assertEquals("io.teknek.nibiru.router.TokenRouter",
                    server.getKeyspaces().get("abc").getKeyspaceMetaData().getRouter().getClass().getName());
            Assert.assertTrue(server.getKeyspaces().get("abc").getStores().containsKey("def"));
        }

    }

    private static MetaDataClient getMetaDataClient(Server[] s) throws ClientException {
    ...
    public void doIt() throws ClientException, InterruptedException {
        ...
        int found = 0;
        for (Server value : s) {
            ColumnValue cv = (ColumnValue) value.get("abc", "def", "a", "b");
    ...
    public void passIfAllAreUp(Server[] s, Session sb) throws ClientException {
        ...
        int found = 0;
        for (Server server : s) {
            ColumnFamilyPersonality c = (ColumnFamilyPersonality) server.getKeyspaces().get("abc")
                    .getStores().get("def");
            ...
            Assert.assertEquals(0, server.getCoordinator().getHinter().getHintsAdded());
        }
        Assert.assertEquals(3, found);
